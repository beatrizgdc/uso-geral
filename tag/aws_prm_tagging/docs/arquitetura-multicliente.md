...
| 3 — Automação contínua | EventBridge + Step Functions (debounce) + Lambda reagindo a criação de recursos, validando tag existente antes de agir | Implementada para 69 dos ~85 serviços do CSV (código + suíte de testes em `test/unit/`, incluindo verificação estrutural contra o botocore real + template SAM em `infra/`) — **validada em sandbox em 2026-09-23/24** (deploy real + eventos CloudTrail reais, 5 bugs corrigidos); uma revisão de código posterior corrigiu mais 3 pontos (revalidação em lote, filtro de `errorCode` no EventBridge, condição `aws:TagKeys`) ainda **pendentes de reconfirmação em sandbox** (só validados estaticamente até aqui). Ver [infra/README.md](../infra/README.md) e [melhorias-futuras.md](melhorias-futuras.md) para o que falta mesclar na stack única e as pendências registradas. |
